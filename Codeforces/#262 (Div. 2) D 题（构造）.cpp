...
构造。
关键在于两个相邻元素的异或可能会很小。
当集合中有两个元素时，答案可能是 l^(l+1)，但 l=1011 时就不太合适，这种情况下 (l+1)^(l+2) 就更好，此时异或为 1。
当集合中有四个元素时，可能出现异或为 0 的情况，也要考虑，方法与有两个元素时类似。
当集合中有三个元素时，唯一可以使异或为 0 的构造是找到一个大于 l 的 2^k 数，令其为 x。
此时异或的结果已经达到最小，更大的集合大小就不用考虑了。
比如 x=10000，那么
x-1=1111，
x+x/2=11000，
x+x/2-1=10111。
此时三者异或恰好为 0，并且这组数恰好是所有三元组中最大数(这里为 x+x/2)最小的。
